...
# AWS Graviton ARM64 Optimized Benchmark Runner (Spack Edition)
# Executes STREAM, LINPACK, and CoreMark with Spack-managed ARM64 NEON optimizations
...
log_info() { echo -e "${BLUE}[GRAVITON-SPACK]${NC} $1"; }
log_success() { echo -e "${GREEN}[SUCCESS]${NC} $1"; }
log_error() { echo -e "${RED}[ERROR]${NC} $1"; }

# Initialize Spack and ARM64 optimized environment
log_info "Loading Spack environment and ARM64 optimized compilers..."
source /opt/spack/share/spack/setup-env.sh
spack load gcc@13
spack load openblas

# Verify ARM64 compilers are loaded
...
RESULTS_FILE="$RESULTS_DIR/graviton_spack_benchmark_results_$(date -u +%Y%m%dT%H%M%SZ).json"
...
# ARM64 NEON-optimized STREAM benchmark
run_stream_graviton() {
    log_info "Running ARM64 NEON-optimized STREAM benchmark..."
...
# OpenBLAS-accelerated LINPACK benchmark for ARM64
...
# ARM64 NEON-optimized CoreMark benchmark
run_coremark_graviton() {
    log_info "Running ARM64 NEON-optimized CoreMark benchmark..."
    ...
    log_info "Executing CoreMark with ARM64 NEON vectorization..."
...
generate_graviton_results() {
    ...
    local cpu_model=$(grep -m1 'model name' /proc/cpuinfo | cut -d: -f2 | sed 's/^ *//' || lscpu | grep "Model name" | cut -d: -f2 | sed 's/^ *//' || echo 'ARM64 CPU')
    local cpu_cores=$(nproc)
    local memory_gb=$(free -g | awk '/^Mem:/ {print $2}')
    
    log_info "Running ARM64-optimized benchmark suite with Spack..."
    ...
    # Generate ARM64-specific JSON results with Spack metadata
    cat << EOF > "$RESULTS_FILE"
{
  "benchmark_metadata": {
    "container_variant": "graviton-arm64-spack-optimized",
    "container_version": "$CONTAINER_VERSION-spack",
    "benchmark_suite": "ARM64 NEON + OpenBLAS accelerated via Spack",
    "execution_timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
    "compiler": "GCC ARM64 managed by Spack",
    "math_library": "OpenBLAS managed by Spack",
    "package_manager": "Spack",
    "optimization_level": "maximum_arm64_performance"
  },
  "spack_environment": {
  ...
  "benchmark_results": {
    "stream_graviton": {
      ...
      "array_size": 80000000,
      "optimization": "ARM64 NEON + GCC native via Spack",
      "status": "completed"
    },
    "linpack_graviton": {
      "gflops": $gflops,
      "math_library": "OpenBLAS ARM64 via Spack",
      "optimization": "OpenBLAS DGEMM + ARM64 threading",
      "status": "completed"
    },
    "coremark_graviton": {
      ...
      "iterations": 100000,
      "optimization": "GCC ARM64 NEON vectorization via Spack",
      "status": "completed"
    }
  },
  "benchmark_summary": {
    ...
    "performance_ratings": {
      ...
      "overall": "ARM64 Optimized via Spack"
    },
    "optimization_benefits": {
      "expected_improvement_over_x86": "Superior power efficiency and cost/performance",
      "neon_acceleration": "ARM64 NEON SIMD for vectorized operations",
      "memory_architecture": "Unified memory architecture advantages",
      "graviton_specific": "Custom silicon optimized for cloud workloads",
...
EOF
    ...
    log_success "=== Graviton ARM64 Spack Benchmark Suite Complete ==="
    log_info "Results saved to: $RESULTS_FILE"
    
    # Display ARM64-specific summary
    echo
    echo "==========================================================="
    echo "      AWS GRAVITON ARM64 BENCHMARK RESULTS (SPACK EDITION)"  
    echo "==========================================================="
    cat "$RESULTS_FILE" | jq -r '
        ...
        "",
        "ARM64 OPTIMIZATIONS VIA SPACK:",
        "✓ Spack-managed ARM64 NEON vectorization",
        "✓ Spack-managed OpenBLAS BLAS acceleration", 
        "✓ Native mcpu optimizations for Graviton",
        "✓ ARM64 unified memory architecture benefits",
        "✓ Reproducible Spack environment",
        "",
        "ARM64 Benefits: " + .benchmark_summary.optimization_benefits.expected_improvement_over_x86'
    echo "==========================================================="
}
...
case "${1:-all}" in
    ...
    "coremark")
        ...
        ;;
    "all"|*)
        log_info "Running complete Graviton ARM64 benchmark suite via Spack..."
        generate_graviton_results
        ;;
esac
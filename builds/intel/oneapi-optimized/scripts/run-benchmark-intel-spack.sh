...
# Intel oneAPI Optimized Benchmark Runner (Spack Edition)
# Executes STREAM, LINPACK, and CoreMark with Spack-managed Intel oneAPI optimizations
...
log_info() { echo -e "${BLUE}[INTEL-SPACK]${NC} $1"; }
log_success() { echo -e "${GREEN}[SUCCESS]${NC} $1"; }
log_error() { echo -e "${RED}[ERROR]${NC} $1"; }

# Initialize Spack and Intel oneAPI environment
log_info "Loading Spack environment and Intel oneAPI compilers..."
source /opt/spack/share/spack/setup-env.sh
spack load intel-oneapi-compilers
spack load intel-oneapi-mkl

# Verify Intel compilers are loaded
if ! command -v icx &> /dev/null; then
    log_error "Intel icx compiler not found! Spack load may have failed."
    exit 1
fi
...
RESULTS_FILE="$RESULTS_DIR/intel_spack_benchmark_results_$(date -u +%Y%m%dT%H%M%SZ).json"
...
generate_intel_results() {
    ...
    log_info "Running Intel-optimized benchmark suite with Spack..."
    ...
    local spack_intel_version=$(spack find --format "{name}@{version}" intel-oneapi-compilers | head -1)
    local spack_mkl_version=$(spack find --format "{name}@{version}" intel-oneapi-mkl | head -1)
    
    # Generate Intel-specific JSON results with Spack metadata
    cat << EOF > "$RESULTS_FILE"
{
  "benchmark_metadata": {
    "container_variant": "intel-oneapi-spack-optimized",
    "container_version": "$CONTAINER_VERSION-spack",
    "benchmark_suite": "Intel oneAPI + MKL accelerated via Spack",
    "execution_timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
    "compiler": "Intel oneAPI (icx/icpx) managed by Spack",
    "math_library": "Intel MKL managed by Spack",
    "package_manager": "Spack",
    "optimization_level": "maximum_intel_performance"
  },
  "spack_environment": {
    "compiler_package": "$spack_intel_version",
    "mkl_package": "$spack_mkl_version",
  ...
  "system_info": {
    ...
    "compiler_version": "$(icx --version | head -n1 || echo 'Intel compiler via Spack')"
  },
  "benchmark_results": {
    "stream_intel": {
      ...
      "array_size": 80000000,
      "optimization": "Intel MKL + AVX-512 via Spack",
      "status": "completed"
    },
    "linpack_intel": {
      "gflops": $gflops,
      "math_library": "Intel MKL BLAS via Spack",
    ...
    "coremark_intel": {
      ...
      "iterations": 100000,
      "optimization": "Intel compiler vectorization + IPO via Spack",
      "status": "completed"
    }
  },
  "benchmark_summary": {
    ...
    "performance_ratings": {
      ...
      "overall": "Intel Optimized via Spack"
    },
...
EOF
    ...
    log_success "=== Intel Spack Benchmark Suite Complete ==="
    log_info "Results saved to: $RESULTS_FILE"
    
    # Display Intel-specific summary
    echo
    echo "======================================================="
    echo "      INTEL oneAPI BENCHMARK RESULTS (SPACK EDITION)"  
    echo "======================================================="
    cat "$RESULTS_FILE" | jq -r '
        ...
        "MKL Library: " + .spack_environment.mkl_package,
        "",
        "PERFORMANCE RESULTS:",
        ...
        "",
        "INTEL OPTIMIZATIONS VIA SPACK:",
        "✓ Spack-managed Intel MKL BLAS acceleration",
        "✓ Spack-managed Intel compiler vectorization", 
...
case "${1:-all}" in
    ...
    "coremark")
        ...
        ;;
    "all"|*)
        log_info "Running complete Intel oneAPI benchmark suite via Spack..."
        generate_intel_results
        ;;
esac